...
def paint_helices(selection="sele", coordfile=""):
	"""
DESCRIPTION

	Colors helices in order of appearance

USAGE

	color_helices [selection [, coord]]

ARGUMENTS

	selection = string: selection to color {default: sele}

	coord = string: file to read for helices
	"""
	#if selection not in pymol.cmd.get_names("all", True): selection = "(all)"
	#if coordfile:
	#	chains = get_helices(coordfile)
	#else: chains = stride(selection)
	#chainlist = chains.keys()
	#for c in chainlist:
	#	helices = chains[c]
	#	colors = gradient(len(helices))
	#	for i in range(len(helices)):
	#		#print("color " + colors[i] + ", i. " + helices[i].chain + str(helices[i]))
	#		#print("color " + str(colors[i]) + ", c. " + helices[i].chain + " and i. " + str(helices[i]))
	#		print(colors[i])
	#		pymol.cmd.color(str(colors[i]), "c. " + helices[i].chain + " and i. " + str(helices[i]))
	#all_chains = get_chains()
	#chainlist = []
	#if type(selection) is dict:
	#	chainlist = selection.keys()
	#elif type(selection) is str:
	#	if selection not in pymol.cmd.get_names("all", True): 
	#		selection = "(all)"
	#if coordfile:
	#	chains = get_helices(coordfile)
	#else: chains = stride(selection)

pymol.cmd.extend("stride", stride)
#pymol.cmd.extend("hsv2rgb", hsv2rgb)
#pymol.cmd.extend("gradient", gradient)
#pymol.cmd.extend("paint_helices", paint_helices)
#pymol.cmd.extend("get_helices", get_helices)
#pymol.cmd.extend("get_chains", get_chains)
pymol.cmd.extend("tms_test", tms_test)
